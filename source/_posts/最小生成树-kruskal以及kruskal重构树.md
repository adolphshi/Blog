...
有 $n$个数 $a_i$，你每次可以选出两个数 $a_i$ 和 $a_j$，获得 {% raw %}$(a_i^{a_j}+a_j^{a_i})\bmod M${% endraw %}  分，并选择这两个数中的一个数删掉，求最大得分。
...
每次删一个点，每个点最多只能被删一次，如果我们将每一次删一个点中被删的点看作儿子，那么我们会发现最终的答案是一个树形结构．而儿子与父亲节点的连边为{% raw %} $(a_i^{a_j}+a_j^{a_i})\bmod M${% endraw %} 因此我们想到两两连边，然后跑最大生成树即可．
...
首先看见异或,可以考虑tire树,我们可以考虑将所有的数插入01tire中,然后进行dfs,类似kruskal,我们可以贪心的发现,两个数字在01tire上分开的深度越深,异或值最小,所以遇见同时有左右儿子的节点,我们可以考虑他的左右儿子,我们向下搜索使它异或值最小的数字(尽可能同向)即可.
